...
//define('COREFRAME_ROOT', dirname(dirname(__FILE__)));
/**
 * 模版编译类
 */
class Template {
    ...
    public function display($templateFile)
    {
        ...
        if(!file_exists($templateFile . '.php')){
            // 没有编译
            $this->cache_template($templateFile);
        }

        include $templateFile . '.php';
    }

    public function includeTpl($templateFile)
    {
        // Does not need this
//        extract(Template::$data);
        ...
        return file_get_contents($templateFile . '.php');
    }
	 ...
	 * @return int|void
	 */
	public function cache_template($template_file){
		if (!file_exists($template_file)) {
//			$template_file = str_replace(COREFRAME_ROOT, '', $template_file);
			throw new Exception($template_file . " is not exists!");
		}
        $cache_path = dirname($template_file);
        if (!is_writable($cache_path)) {
            throw new Exception($cache_path . ' 目录不可写');
        }
		...
		$cache_file = $cache_path . '/' . basename($template_file) . '.php';
		...
		if ($templatelen == false) throw new Exception($cache_file . ' is readonly!');
	...
	public function template_parse($template){
		...
		$template = preg_replace("/\{foreach\s+(\S+)\s+(\S+)\}/", "<?php \$n=1;if(is_array(\\1)) foreach(\\1 AS \\2) { 
		?>",
            $template);
		$template = preg_replace("/\{foreach\s+(\S+)\s+(\S+)\s+(\S+)\}/", "<?php \$n=1; if(is_array(\\1)) foreach(\\1 AS \\2 => \\3) { ?>", $template);
		$template = preg_replace("/\{\/foreach\}/", "<?php \$n++;}?>", $template);
		...
		$template = "<?php defined('IN_CMS') or exit('No direct script access allowed'); ?>\n" . $template;
		return $template;

	}

    private static function parseInclude($matches)
    {
        $tplFile =  tpl_dir . $matches[1];
        // include the file
        if(is_file($tplFile)){
            $tpl = new Template();
            return $tpl->includeTpl($tplFile);
        }
    }
